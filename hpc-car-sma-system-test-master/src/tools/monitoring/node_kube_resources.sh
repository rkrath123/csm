...
#	$KUBECTL get pods -owide
	...
	echo
	for n in $nodes; do
		local requests=$($KUBECTL describe node $n | grep -A2 -E "\\s*Requests" | tail -n1)
		local percent_cpu=$(echo $requests | awk -F "[()%]" '{print $2}')

		local requests=$($KUBECTL describe node $n | grep -A3 -E "\\s*Requests" | tail -n1)
		local percent_mem=$(echo $requests | awk -F "[()%]" '{print $2}')
