		...
		if(extraDataLen == 0) {
			if(useRFC5424Format) {
				...
				/* <priority>VERSION ISOTIMESTAMP HOSTNAME APPLICATION PID MESSAGEID STRUCTURED-DATA MSG */
  				*pLenBuf = snprintf(buf, maxBuf, "<%s>1 %s "
 						"%s tcpflood 32473 "
 						"MSGNUM=\"%8.8d\" msgnum:%s%8.8d%c",
 						msgPRI, timestamp, hostname, msgNum, dynFileIDBuf, msgNum, frameDelim);
...
int main(int argc, char *argv[])
{
	...
	time(&now);
	ts = *localtime(&now);
	strftime(timestamp, sizeof(buf), "%Y-%m-%dT%H:%M:%S.000Z", &ts);
/*	strncpy(timestamp, "2019-06-01T07:00:00.000Z", sizeof(timestamp));
