...
function usage()
{
    ...
    echo "System Monitoring Application verifies the notification pod is Running."
    echo "$0 > sma_component_monasca_notification_pod-\`date +%Y%m%d.%H%M\`"
    echo
    exit 1
}
...
echo
echo "Monasca notification pod is in the expected state"
