...
function usage()
{
    ...
    echo "test checks for the grafana datasource."
    echo "$0 > sma_component_grafana_datasource-\`date +%Y%m%d.%H%M\`"
    echo
    exit 1
}
...
#  	Checks for the grafana datasource.
...
echo
echo "Grafana datasource looks good"
