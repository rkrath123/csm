...
function usage()
{
    ...
    echo "tests that the auditlog pods are running."
    echo "$0 > sma_component_rsyslog_auditlog_pods-\`date +%Y%m%d.%H%M\`"
    echo
    exit 1
}
...
echo
echo "Rsyslog auditlog pod is in the expected state"
